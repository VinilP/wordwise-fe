...
import { createMockReview, createMockUser, createMockBook } from '@/test/utils/test-utils';
...
describe('Review Management Workflow Integration Tests', () => {
  ...
  describe('Review Creation Workflow', () => {
    ...
    it('should handle review creation errors', async () => {
      const user = userEvent.setup();
      const mockOnSubmit = vi.fn().mockRejectedValue(new Error('Failed to create review'));
  ...
  describe('Review List Workflow', () => {
    it('should display reviews with proper information', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Amazing book! Highly recommended.')).toBeInTheDocument();
        expect(screen.getByText('Good read, but could be better.')).toBeInTheDocument();
  ...
  describe('Review Card Workflow', () => {
    ...
    it('should show edit/delete buttons for user\'s own reviews', () => {
      ...
      render(<ReviewCard review={review} onEdit={mockOnEdit} onDelete={mockOnDelete} />, { 
        queryClient,
        initialAuthState: { user: mockUser, isAuthenticated: true }
      });
