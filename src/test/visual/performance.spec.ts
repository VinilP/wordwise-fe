...
class PerformanceTester {
      ...
      return new Promise<PerformanceMetrics>((resolve) => {
        ...
        try {
          ...
          if (lcpEntries.length > 0) {
            collectedMetrics.largestContentfulPaint =
          ...
          if (navigationEntry) {
            collectedMetrics.totalBlockingTime = Math.max(
              0,
              navigationEntry.loadEventEnd -
                navigationEntry.domContentLoadedEventEnd,
            );
            collectedMetrics.timeToInteractive =
            ...
            const observer = new PerformanceObserver((list) => {
              ...
              entries.forEach((entry) => {
                if (entry.entryType === "first-input") {
                  collectedMetrics.firstInputDelay = Math.max(
                    0,
                    (entry as any).processingStart - entry.startTime,
                  );
                } else if (entry.entryType === "layout-shift") {
                  if (!(entry as any).hadRecentInput) {
                    collectedMetrics.cumulativeLayoutShift =
                      (collectedMetrics.cumulativeLayoutShift || 0) +
                      (entry as any).value;
                  }
                }
              });
  ...
  async measureBundleSize(
    page: Page,
  ): Promise<{ jsSize: number; cssSize: number; totalSize: number }> {
...
test.describe("Performance Testing Suite", () => {
  ...
  test("Books Page Performance Test", async () => {
    const metrics = await performanceTester.measurePagePerformance(
      page,
      "/books",
    );
  ...
  test("API Response Time Test", async () => {
    ...
    for (const endpoint of endpoints) {
      const responseTime = await performanceTester.measureAPIResponseTime(
        page,
        `${baseURL}${endpoint}`,
      );
  ...
  test("Image Loading Performance Test", async () => {
    ...
    const _imageLoadTimes = await page.evaluate(() => {
  ...
  test("Concurrent User Performance Test", async ({ browser }) => {
    ...
    const averageMetrics = allMetrics.reduce(
      (acc, metrics) => {
