...
describe('ReviewForm', () => {
  ...
  it('calls onSubmit with correct data for create form', async () => {
    ...
    // Set content
    const textarea = screen.getByPlaceholderText('Share your thoughts about this book...');
    fireEvent.change(textarea, { target: { value: 'This is a great book with excellent characters.' } });
  ...
  it('calls onSubmit with correct data for update form', async () => {
    ...
    // Update content
    const textarea = screen.getByDisplayValue('Initial review content that is long enough.');
    fireEvent.change(textarea, { target: { value: 'Updated review content that is also long enough.' } });
  ...
  it('disables form when isLoading is true', () => {
    render(<ReviewForm bookId="book-1" onSubmit={mockOnSubmit} isLoading={true} />);
