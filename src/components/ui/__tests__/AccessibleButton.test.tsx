...
describe('AccessibleButton', () => {
  ...
  it('renders with different variants', () => {
    ...
    button = screen.getByRole('button');
    expect(button).toHaveClass('text-gray-700', 'hover:bg-gray-100', 'focus:ring-gray-500');
  });
  ...
  it('shows loading state', () => {
    render(<AccessibleButton loading loadingText="Saving...">Save</AccessibleButton>);
    ...
    expect(screen.getByText('Saving...', { selector: '.sr-only' })).toBeInTheDocument();
    expect(screen.getByText('Save', { selector: '.sr-only' })).toBeInTheDocument();
  });
  ...
  it('prevents click when loading', async () => {
    ...
    render(<AccessibleButton loading onClick={handleClick}>Loading</AccessibleButton>);
  ...
  it('applies custom className', () => {
    render(<AccessibleButton className="custom-button-class">Custom</AccessibleButton>);
  ...
  it('handles custom onKeyDown', async () => {
    ...
    render(<AccessibleButton onKeyDown={handleKeyDown}>Key test</AccessibleButton>);
  ...
  it('shows correct loading text', () => {
    render(<AccessibleButton loading loadingText="Processing...">Process</AccessibleButton>);
    
    expect(screen.getByText('Processing...', { selector: '.sr-only' })).toBeInTheDocument();
    expect(screen.getByText('Process', { selector: '.sr-only' })).toBeInTheDocument();
  });
  ...
  it('handles loading state with custom loading text', () => {
    ...
    const button = screen.getByRole('button');
    expect(button).toHaveAttribute('aria-busy', 'true');
    expect(screen.getByText('Uploading file...', { selector: '.sr-only' })).toBeInTheDocument();
  });
});
