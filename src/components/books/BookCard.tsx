...
export const BookCard: React.FC<BookCardProps> = ({ book, className = '' }) => {
  ...
  return (
    <div className={`tooltip group relative w-full ${className}`} data-testid="book-card">
        ...
        >
          ...
          {imageLoading && (
            <div className="absolute inset-0 flex items-center justify-center bg-gray-100">
              <LoadingSpinner size="md" text="Loading..." className="text-gray-400" />
            </div>
          )}
        ...
        <div className="p-4">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-1">
              <div className="flex" role="img" aria-label={`${getRatingValue(book.averageRating)} out of 5 stars`}>
                {renderStars(book.averageRating)}
              </div>
              {hasRating(book.averageRating) && (
                <span className="text-sm text-gray-600 ml-1" aria-label={`${getReviewCount(book.reviewCount)} reviews`}>
            ...
            {hasRating(book.averageRating) && (
              <span className="text-sm font-medium text-gray-700" aria-label={`Average rating: ${getRatingValue(book.averageRating)}`}>
                {getRatingValue(book.averageRating)}
              </span>
            )}
          </div>
          
          <div className="mt-2 flex flex-wrap gap-1" role="list" aria-label="Book genres">
